...
#
# CÓDIGO MODIFICADO:
# Explicação: A importação do njit foi mantida para outras funções (como motor_ode_system e
# calculate_electric_torque), mas o decorador foi removido da função run_motor_time_loop
# para permitir chamadas dinâmicas a objetos Python puros.
from numba import njit
...
#
# CÓDIGO MODIFICADO:
# Explicação: Inclusão do InverterFOC na lista de imports de inversores para instanciamento
# no novo método run_with_inverter_foc.
...
class MotorElement(Element):
    ...
    #
    # CÓDIGO MODIFICADO:
    # Explicação: Adicionado o argumento opcional `wref` para fornecer a referência
    # de velocidade em casos de uso de InverterFOC em malha fechada.
    def run(
        self,
        t,
        time_step=None,
        ...
        wref : float, optional
            Shaft speed reference in cases that InverterFOC has been applied, 
            perfoming closed loop control. 
        ...
        wr0 = 0.0
        thetar0 = 0.0
        flux_angle_0 = self.initial_angle_net - np.pi / 2

        # Initial alpha-beta and dq currents
        ias, ibs, ics = 0, 0, 0
        ...
        #
        # CÓDIGO MODIFICADO:
        # Explicação: Verifica se o elemento inserido atua em malha fechada (InverterFOC).
        # Se for malha fechada, os arrays de tensão são inicializados com zero e a matriz 
        # wref_arr de controle é montada. Se for malha aberta (InverterVF / SourceAC), 
        # os valores são pré-computados integralmente pela função np.vectorize, como anteriomente.
        closed_loop = type(element).__name__ == "InverterFOC"
        ...
        #     t_simul, dt, flux_angle_0, Lds0, Lqs0, Ldr0, Lqr0, wr0,
        #     load_torque_entrance_time, Tl_full, w_shaft, vas, vbs, vcs,
        #     self.a, self.b, self.c, self.n_poles, Rs, self.rotor_resistance,
        #     self.viscosity_coeff, Ip,
        # )
        #
        # CÓDIGO MODIFICADO:
        # Explicação: O método run_motor_time_loop recebe novos parâmetros (element,
        # closed_loop e wref_arr) para permitir iterações de simulação passo a passo
        # dentro de lógicas realimentadas.
        outputs = run_motor_time_loop(
            t_simul,
            dt,
            flux_angle_0,
    ...
    @check_units
    def run_with_AC_source(self, t, time_step=None, load_torque_entrance_time=None, load_torque_ratio=1.0, voltage_net=None, frequency_net=None, initial_phase_angle=0.0, harmonics=None, unbalances=None):
        if harmonics:
    ...
    @check_units
    def run_with_inverter(self, t, time_step=None, load_torque_entrance_time=None, load_torque_ratio=1.0, frequency_s=None, time_ramp=1.0, frequency_ref=None):
    ...
    # CÓDIGO ORIGINAL:
    # (Não existia implementação direta de run_with_inverter_foc no arquivo)
    #
    # CÓDIGO MODIFICADO:
    # Explicação: Função implementada para abstrair a instância do objeto InverterFOC, garantindo
    # os parâmetros compatíveis (como torque_nom, speed_nom) e invocando o laço principal através de run().
    @check_units
    def run_with_inverter_foc(
        ...
        """Simulate motor with variable frequency inverter controlled via iFOC.

        Runs motor simulation with a three-phase voltage source inverter employing
        Space Vector PWM modulation and indirect Field-Oriented Control (iFOC).
        The inverter dynamically computes the generated voltages iteratively in a
        closed control loop.
        ...
        wref : float, array-like or pint.Quantity, optional
            Reference mechanical speed [rad/s]. Default is nominal speed.
        """
        Vnl = phase_to_line(self.voltage_nom)
        line_to_dc_bus = 1.35
        
        Ts = 2 * np.pi / frequency_s
        sim_time_step = time_step or Ts / 200
...
#
# CÓDIGO MODIFICADO:
# Explicação: O decorador @njit foi removido e os argumentos da função foram estendidos 
# (element, closed_loop e wref_arr). O interpretador nativo é exigido pois o Numba não 
# permite chamadas dinâmicas a classes Python complexas (como é o caso do element.get_operating_state) 
...
def run_motor_time_loop(
    ...
    for step in range(1, nt):
        ...
        #
        # CÓDIGO MODIFICADO:
        # Explicação: Em laço fechado, as correntes reais são computadas diretamente das variáveis 
        # Lds e Lqs vigentes e, em seguida, convertidas via Park (inversa) e repassadas ao objeto 
        # InverterFOC junto a velocidade wr. O elemento responde computando as tensões atuantes para 
        # aquele passo, que atualizam simultaneamente a estrutura de memória dos arrays.
        if closed_loop:
            ids_val = a * Lds - c * Ldr
            iqs_val = a * Lqs - c * Lqr
            ...
            w_shaft_val, vas_val, vbs_val, vcs_val = element.get_operating_state(
                t, wref=wref_arr[step], wr=wr, ia=ias, ib=ibs, ic=ics, dt=dt
            )
            
            w_shaft_arr[step] = w_shaft_val
            vas_arr[step] = vas_val
            vbs_arr[step] = vbs_val
            vcs_arr[step] = vcs_val
